...
### Issue in Read commited isolation level
```
// pseudo code
BEGIN;

let balance = SELECT balance FROM wallet WHERE user = A;
...
### Problem
- Two requests come at same time:
- T1:
- reads balance = 100
- T2:
- reads balance = 100
- Both pass condition → both deduct
- Final result:
- balance = -100 ❌
- 👉 This is called:
- Lost update problem
...
- higher isolation to use , in order to fix non-repeatable read
...
- above as we can see two consecutive read can result in different value and hence can hamper the decision making , hence in this case snapshot isolation will help
