...
### TCP (used by HLS/DASH)
- ACK for packets
- Retransmissions
- Higher latency
- So logically:
- “UDP should be better for live streaming”
- ✔️ This is true for real-time communication
- ❌ But not optimal for large-scale video streaming

### The Real Problem: Internet Reality
- When you stream to millions of users, problems change:
- ❌ UDP Problems at Scale
- Packet loss → no recovery
- No congestion control → network collapse risk
- Blocked by firewalls / NATs (very common)
- No built-in ordering
- Hard to cache (very important!)
- 👉 Result:
- Bad user experience (buffering, glitches, broken streams)

### Why HLS/DASH (TCP) Win for Streaming
- Reliability matters more than raw speed
- Video streaming is not truly real-time:
  - If 1 frame is lost → video glitches
  - TCP retransmits → smoother playback
- 👉 Users prefer:
> Slight delay + smooth video
> over
> Instant but glitchy video

### CDN Compatibility (BIGGEST REASON)
- Protocols like HLS/DASH work over HTTP → can use CDNs like:
  - Cloudflare
  - Akamai
- CDN advantages:
  ...
  - Scale to millions
- 👉 UDP-based streaming:
  - ❌ Cannot leverage CDNs effectively

### Adaptive Bitrate Streaming (ABR)
- HLS/DASH can:
- Detect network speed
- Switch quality dynamically (1080p → 480p)
- 👉 This is critical for mobile networks
- UDP alone doesn’t provide this out of the box.

### Works Everywhere
- TCP/HTTP:
  - Works behind NAT, proxies, firewalls
  - No special setup
- UDP:
  - Often blocked or throttled
  - Requires STUN/TURN (complex infra)
 
### When UDP is Better (You’re Right Here)
- UDP is used in:
  - 🔹 WebRTC
...
- 👉 Why it works here:
  ...
  - Prioritizes real-time over perfection

### Key Trade-off (THIS is your interview gold line)
- 👉 TCP (HLS/DASH)
-  Reliability + scalability + CDN + compatibility
-  Higher latency (~5–30s)
- 👉 UDP (WebRTC)
  -  Low latency + real-time
  -  Hard to scale + unreliable at large scale
